...
$departure_time = date('Y-m-d\TH:i:s', strtotime($_POST['departure_time']));
$qry = $conn->query("SELECT sl.*, d.departing_from, d.landing_to FROM schedule_list AS sl join departures AS d ON sl.depart_id = d.id where sl.depart_id = '" . $_POST['depart_id'] . "' AND sl.depart_time >=  '" . $departure_time . "' and sl.`status` = 1");
$data = array();
