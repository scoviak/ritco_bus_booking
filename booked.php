     ...
     <main id="main">
         <div class="container-fluid">
             <div class="col-lg-12">
                 ...
                 <div class="row">
                     <div class="col-md-12">
                         <a href="?page=all_booked" class="btn btn-primary btn-sm" type="button" id="new_schedule">Load
                             All Bookings</a>
                     </div>
                 </div>
                 <?php endif; ?>
...
window.load_booked = function() {
    ...
    $.ajax({
                ...
                if (typeof(resp) != undefined) {
                    ...
                    if (Object.keys(resp).length > 0) {
                        ...
                        Object.keys(resp).map(k => {
                            ...
                            tr.append('<td class="">' + (resp[k].status == 1 ? 'Paid' :
                                'Unpaid') + '</td>')

                            tr.append(
                                '<td><center><button class="btn btn-sm btn-primary mr-2 text-white edit_booked" data-id="' +
                                resp[k].schedule_id + '" data-bid="' + resp[k].id +
                                '"><strong>Edit</strong></button></center></td>')
        ...
        complete: function() {
            ...
            $('body').on('click', '.edit_booked', function() {
                uni_modal('Edit Booked', 'customer_book.php?id=' + $(this).attr('data-id') +
                    '&bid=' + $(this).attr('data-bid'), 1)
            })
        }
    })
}
